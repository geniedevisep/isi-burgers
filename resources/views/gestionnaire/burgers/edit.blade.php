...
<head>
    ...
    <title>ISI BURGER - Modifier un Burger</title>
    ...
    <div id="main-content">
        <div class="container-fluid">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h1 class="h3">Modifier le burger</h1>
            ...
            <div class="form-card p-4">
                <form action="{{ route('gestionnaire.burgers.update', $burger) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('nom') is-invalid @enderror" 
                                       id="nom" name="nom" value="{{ old('nom', $burger->nom) }}" required>
                                @error('nom')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" step="0.01" class="form-control @error('prix') is-invalid @enderror" 
                                       id="prix" name="prix" value="{{ old('prix', $burger->prix) }}" required>
                                @error('prix')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" 
                                          id="description" name="description" rows="3" required>{{ old('description', $burger->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="categorie" id="categorie" class="form-control @error('categorie') is-invalid @enderror" required>
                                    <option value="classic" {{ $burger->categorie == 'classic' ? 'selected' : '' }}>Classic</option>
                                    <option value="signature" {{ $burger->categorie == 'signature' ? 'selected' : '' }}>Signature</option>
                                    <option value="veggie" {{ $burger->categorie == 'veggie' ? 'selected' : '' }}>Veggie</option>
                                </select>
                                @error('categorie')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="archived" name="archived" 
                                           value="1" {{ $burger->archived ? 'checked' : '' }}>
                    ...
                    <div class="d-grid gap-2 mt-4">
                        <button type="submit" class="btn btn-primary">
                            <i class="fas fa-save"></i> Mettre à jour le burger
                        </button>
                    </div>
                </form>
            </div>
